...
from keyboards import after_teacher_keyboard
...
async def teacher_query_entered(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Number → select from previous list
    ...
    # Single result — load immediately
    if len(results) == 1:
...
async def _load_teacher_schedule(update: Update, teacher: dict) -> int:
    ...
    await update.message.reply_text("Что дальше?", reply_markup=after_teacher_keyboard())
    return MAIN_MENU
